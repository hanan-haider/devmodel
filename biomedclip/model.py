...
>>>>>>> a72473ac39070f418c26f7d94932cfe96b138714
def resize_pos_embed_biomedclip(state_dict, model, interpolation='bicubic', antialias=True):
    ...
    # shape: (1, 197, 768)
    old_pos_embed = old_pos_embed[0]  # remove batch dim → (197, 768)

    # detect new grid size from model
    grid_h, grid_w = model.visual.grid_size  
    new_seq_len = grid_h * grid_w + 1   # +1 for CLS token

    if new_seq_len == old_pos_embed.shape[0]:
        return  # nothing to change
    
    # split cls + patch embeddings
    cls_token = old_pos_embed[:1]       # (1, 768)
    patch_tokens = old_pos_embed[1:]    # (196, 768)

    # old grid size
    old_grid = int(math.sqrt(patch_tokens.shape[0]))
    old_grid = (old_grid, old_grid)

    # reshape → interpolate → reshape back
    patch_tokens = patch_tokens.reshape(1, old_grid[0], old_grid[1], -1).permute(0, 3, 1, 2)
    patch_tokens = F.interpolate(
        patch_tokens,
        size=(grid_h, grid_w),
        mode=interpolation,
    ...
    patch_tokens = patch_tokens.permute(0, 2, 3, 1).reshape(grid_h * grid_w, -1)

...
#printing the arguments 
  ...
  #for printing at the outset of training
    ...
    for epoch in range(args.epoch):
        ...
        if result > best_result + min_delta:
            best_result = result
            patience_counter = 0
            print(" Best result - Saving model\n")
            ...
            if patience_counter >= patience_limit:
                ...
                print(f"Best result: {best_result:.4f} (from epoch {epoch - patience_counter})")
                print(f"{'='*60}\n")
                break
...
def test(args, model, test_loader, text_features, seg_mem_features, det_mem_features):
    ...
    for (image, y, mask) in tqdm(test_loader):
        image = image.to(device)
        batch_size = image.shape[0]  # ✅ Get actual batch size
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            for i in range(batch_size):
                # Extract tokens for this single image
                ...
                # ✅ Append mask for THIS image only
                gt_mask_list.append(mask[i].squeeze().cpu().detach().numpy())
            
            # ✅ Extend gt_list with batch labels
